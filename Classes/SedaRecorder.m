...
#define FILTERS_DELAY (BARKER_LEN+2)*SAMPLE_PER_BIT
    ...
    NSString *message = @"No Signal Detected!\0";
    NSUInteger len = [message length];
    for(int i = 0; i < len; i++) {
        strbuf[i] = [message characterAtIndex:i] ;
    }
}

    [(SedaRecorder *)pRecordState->mSelf performSelectorOnMainThread:@selector(updateTextView) withObject:nil waitUntilDone:NO];
    ...
    if (pRecordState->mIsRunning) {
        [(SedaRecorder *)pRecordState->mSelf stopRecording];
    }
    
}

@interface SedaRecorder (Private)
- (void)_setupAudioFormat;
- (void)_deriveBufferSize:(Float64)seconds;
@end

@implementation SedaRecorder
- (void)dealloc {
...
@implementation SedaRecorder (Private)
- (void)_setupAudioFormat {
